...
export const PART_INFO = {
  block:   { name: '缸体 Block', desc: '铝合金低压铸造的发动机骨架,内部四个气缸孔经珩磨加工,活塞在其中往复运动;下部曲轴箱装五道主轴承座,水套与油道贯穿整个缸体。' },
  head:    { name: '缸盖 Cylinder Head', desc: '位于缸体顶部,通过缸垫与缸体密封。内置 pent-roof 燃烧室与进排气道,双顶置凸轮轴(DOHC)经挺柱直接压动 16 气门,控制换气过程。' },
  piston:  { name: '活塞 Piston', desc: '承受燃烧爆发的压力,在气缸内往复运动。顶部是浅盆型燃烧室底面,三道环槽中的活塞环负责密封燃气并刮布机油,活塞销连接连杆。' },
  rod:     { name: '连杆 Connecting Rod', desc: '连接活塞与曲轴的传力杆件,"工"字形截面在轻量与刚性间取得平衡。大头端为剖分式结构,内装轴瓦,把往复运动转换为旋转运动。' },
  crank:   { name: '曲轴 Crankshaft', desc: '发动机的"脊梁"。锻钢曲轴把四个气缸的往复力汇聚成旋转扭矩输出;曲柄臂延伸出扇形配重块平衡离心力,五道主轴颈支撑,尾部装飞轮。' },
  cam:     { name: '凸轮轴 Camshaft', desc: '以曲轴一半的转速旋转(2:1)。每根轴上 8 个凸轮桃尖按 1-3-4-2 点火顺序准时下压气门,实现进气与排气的正时控制。' },
  valve:   { name: '气门 Valves', desc: '进气门开启吸入新鲜空气,排气门开启排出废气。盘状头部与气门座圈贴合密封,气门弹簧保证它及时回座。每缸 4 气门,共 16 个。' },
  plug:    { name: '火花塞 Spark Plug', desc: '在压缩行程终了释放电火花,点燃被活塞压缩的可燃混合气,推动活塞下行做功——点火系统的执行端。' },
  gasket:  { name: '缸盖垫片 Head Gasket', desc: '夹在缸体与缸盖之间的多层金属(MLS)密封垫,既要封住数千度的高温燃气,又要封住水道与油道,是发动机最苛刻的密封件。' },
  intake:  { name: '进气歧管 Intake Manifold', desc: '稳压腔先把空气"存"起来,再通过四根弯曲的进气道均分给四个气缸。气道长度经过调校,利用脉冲效应提升中低转扭矩。' },
  exhaust: { name: '排气歧管 Exhaust Manifold', desc: '把四个气缸排出的高温废气汇集到一起送往三元催化器。工作时温度可超过 800°C,由耐热不锈钢制成。' },
  pan:     { name: '油底壳 Oil Pan', desc: '发动机最底部的机油储存池,带深海造型集油槽。曲轴旋转时曲柄会溅起机油润滑缸壁,底部磁性放油螺栓用于换油。' },
  flywheel:{ name: '飞轮 Flywheel', desc: '安装在曲轴尾部的大惯量圆盘,储存做功行程的能量,填补其他三个行程的"动力空窗",平息扭矩脉动。外圈齿圈供起动机啮合。' },
  pulley:  { name: '曲轴皮带轮 Crank Pulley', desc: '曲轴前端的旋转输出端,通过多楔带驱动发电机、空调压缩机等附件,内含扭转减振橡胶层。' },
  belt:    { name: '正时链条 Timing Chain', desc: '严格锁定曲轴与凸轮轴 2:1 的相位关系——正时一旦错位,气门就会与活塞相撞,发动机大修。' },
};

// ---------- 参数 ----------
const R = 0.42;            // 曲柄半径(半冲程)
const L = 1.55;            // 连杆长度
const BORE = 0.46;         // 缸孔半径
const SPACING = 1.6;       // 缸心距
const ZPOS = [-2.4, -0.8, 0.8, 2.4];
const PHASE = [0, Math.PI, Math.PI, 0];              // 曲柄相位(平面曲轴)
const FIRE = [0, Math.PI, 1.5 * Math.PI, 0.5 * Math.PI]; // 点火间隔
const DECK = R + L + 0.42;  // 缸体顶面 ≈ 2.39
const HALFLEN = 3.5;
const CAM_Y = DECK + 1.02;
const CAM_X = 0.27;
const VALVE_X = 0.27;
const MAXLIFT = 0.13;
...
// 打孔板(Shape 带孔 → ExtrudeGeometry), 用于 MLS 缸垫
function plateWithHoles(w, d, holes, thick, mat) {
...
export function buildEngine() {
  ...
  const reg = (name, group, dir, dist) => {
    ...
    const set = new Set();
    group.traverse(o => o.isMesh && o.material && set.add(o.material));
    mats[name] = [...set];
  ...
  // 共享材质
  const bm = M(0x4a5158, 0.62, 0.65);          // 缸体铝
  const hm = M(0x5b636c, 0.55, 0.7);           // 缸盖铝
  const cm = M(0xb8bec6, 0.32, 0.95);          // 曲轴钢
  const dark = M(0x23262b, 0.4, 0.85);         // 深色机加工面
  const linerMat = M(0x2a2e33, 0.35, 0.9);
  ...
  const front = new THREE.Mesh(new THREE.BoxGeometry(BW, DECK + 0.75, 0.14), bm);
  ...
  for (const zt of [-3.2, -1.6, 0, 1.6, 3.2]) {
    const strip = new THREE.Mesh(new THREE.BoxGeometry(BW, 0.14, 0.42), bm);
  ...
  // 缸套(从顶面可见的珩磨缸孔)
  for (const z of ZPOS) {
    const liner = new THREE.Mesh(new THREE.CylinderGeometry(BORE + 0.03, BORE + 0.03, DECK + 0.2, 32, 1, true), linerMat);
    liner.position.set(0, (DECK - 0.75) / 2 + 0.1, z); block.add(liner);
  }
  // 主轴承座(曲轴箱下缘 5 道)
  for (const zp of [-2.4, -1.2, 0, 1.2, 2.4]) {
    const mb = new THREE.Mesh(new THREE.BoxGeometry(BW, 0.34, 0.3), bm);
    mb.position.set(0, -0.92, zp); block.add(mb);
    const cap = new THREE.Mesh(new THREE.CylinderGeometry(0.24, 0.24, 0.18, 20), dark);
    cap.rotation.x = Math.PI / 2; cap.position.set(0, -0.92, zp);
    block.add(cap);
  }
  ...
  const gm = M(0x9aa2ab, 0.42, 0.9);
  const rubber = M(0x35435a, 0.6, 0.4);
  const holes = ZPOS.map(z => [0, z, BORE + 0.06]);
  // 打孔主板
  const main = plateWithHoles(1.3, HALFLEN * 2, holes, 0.035, gm);
  main.position.y = DECK + 0.085;
  gasket.add(main);
  // 缸孔周圈蓝色密封圈 + 4 个油道孔
  for (const z of ZPOS) {
    const ring = new THREE.Mesh(new THREE.TorusGeometry(BORE + 0.07, 0.022, 8, 32), rubber);
    ring.rotation.x = -Math.PI / 2; ring.position.set(0, DECK + 0.105, z);
    gasket.add(ring);
  }
  for (const z of ZPOS) {
    const oil = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 0.05, 12), gm);
    oil.position.set(0.55, DECK + 0.09, z + 0.6); gasket.add(oil);
  }
  reg('gasket', gasket, [0, 1.3, 0], 1);

  // ================= 缸盖 =================
  const head = new THREE.Group();
  const hBase = new THREE.Mesh(new THREE.BoxGeometry(BW, 0.34, HALFLEN * 2), hm);
  ...
  const hFront = new THREE.Mesh(new THREE.BoxGeometry(BW, 0.85, 0.14), hm);
  ...
  for (let j = 0; j < 4; j++) {
    ...
    const pin = new THREE.CylinderGeometry(0.16, 0.16, 0.34, 18);
    const pinMesh = new THREE.Mesh(pin, cm);
    pinMesh.rotation.x = Math.PI / 2;
    pinMesh.position.set(-R * Math.sin(th), R * Math.cos(th), ZPOS[j]);
    crank.add(pinMesh);
    // 扇形配重块(反向于曲柄销)
    for (const dz of [-0.44, 0.44]) {
      const cwShape = new THREE.Shape();
      cwShape.absarc(0, 0, 0.52, Math.PI * 1.15, Math.PI * 1.85, false);
      cwShape.closePath();
      const cwGeo = new THREE.ExtrudeGeometry(cwShape, { depth: 0.16, bevelEnabled: false });
      const cw = new THREE.Mesh(cwGeo, cm);
      cw.rotation.x = Math.PI / 2;
      cw.position.set(0, 0, ZPOS[j] + dz);
      // 配重扇形朝向曲柄销反方向: 绕 z 轴转 th+π, 再整体转 x90 后是绕 y
      cw.rotation.z = 0;
      const holder = new THREE.Group();
      holder.add(cw);
      holder.rotation.z = th;
      holder.position.set(0, 0, ZPOS[j] + dz);
      crank.add(holder);
    }
    // 连杆大头轴承(红色轴瓦示意)
    const bearing = new THREE.Mesh(new THREE.CylinderGeometry(0.175, 0.175, 0.3, 18), M(0x8a4a3b, 0.5, 0.6));
    bearing.rotation.x = Math.PI / 2;
    bearing.position.set(-R * Math.sin(th), R * Math.cos(th), ZPOS[j]);
    crank.add(bearing);
  }
  // ----- 飞轮(带齿圈) -----
  const fwM = M(0x55606b, 0.45, 0.85);
  const flywheel = new THREE.Mesh(new THREE.CylinderGeometry(1.02, 1.02, 0.2, 48), fwM);
  flywheel.rotation.x = Math.PI / 2; flywheel.position.z = -HALFLEN - 0.55;
  // 齿圈:细齿环
  const gearM = M(0x39424c, 0.5, 0.8);
  const teeth = 48;
  for (let i = 0; i < teeth; i++) {
    const a = i / teeth * Math.PI * 2;
    const tooth = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.075, 0.12), gearM);
    tooth.position.set(Math.cos(a) * 1.06, Math.sin(a) * 1.06, -HALFLEN - 0.55);
    tooth.rotation.z = a;
    crank.add(tooth);
  }
  const fwRing = new THREE.Mesh(new THREE.TorusGeometry(1.02, 0.07, 10, 48), gearM);
  fwRing.position.z = -HALFLEN - 0.55;
  const hub = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.3, 0.34, 20), cm);
  hub.rotation.x = Math.PI / 2; hub.position.z = -HALFLEN - 0.5;
  // 飞轮减重孔
  for (let i = 0; i < 6; i++) {
    const a = i / 6 * Math.PI * 2;
    const hole = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.09, 0.22, 12), dark);
    hole.rotation.x = Math.PI / 2;
    hole.position.set(Math.cos(a) * 0.62, Math.sin(a) * 0.62, -HALFLEN - 0.55);
    crank.add(hole);
  }
  ...
  for (let j = 0; j < 4; j++) {
    ...
    const bowl = new THREE.Mesh(new THREE.SphereGeometry(BORE - 0.09, 24, 12, 0, Math.PI * 2, 0, Math.PI / 2), dark);
    bowl.rotation.x = Math.PI; bowl.position.y = 0.26;
    for (const ry of [0.22, 0.15, 0.07]) {   // 三道环
    ...
    // 工字截面连杆:窄腰 + 上下加强缘
    const beam = new THREE.Mesh(new THREE.BoxGeometry(0.07, L - 0.34, 0.13), rodMat);
    beam.position.y = L / 2;
    const flangeF = new THREE.Mesh(new THREE.BoxGeometry(0.13, L - 0.34, 0.045), rodMat);
    flangeF.position.set(0, L / 2, 0.06);
    const flangeB = flangeF.clone(); flangeB.position.z = -0.06;
    const bigEnd = new THREE.Mesh(new THREE.TorusGeometry(0.21, 0.075, 12, 26), rodMat);
    bigEnd.rotation.y = Math.PI / 2;
    const bigCap = new THREE.Mesh(new THREE.CylinderGeometry(0.215, 0.215, 0.16, 24), rodMat);
    bigCap.rotation.x = Math.PI / 2;
    ...
    rg.add(beam, flangeF, flangeB, bigEnd, bigCap, smallEnd);
    rg.userData.j = j;
    root.add(rg);
  ...
  const panTop = new THREE.Mesh(new THREE.BoxGeometry(BW, 0.3, HALFLEN * 2), pm);
  panTop.position.y = -0.9;
  const sump = new THREE.Mesh(new THREE.BoxGeometry(BW, 0.55, 3.4), pm);
  ...
  arc(0, 0, 0.52, -Math.PI / 2, Math.PI / 3, 10);
  pts.push([0.415, 3.19]);
  arc(CAM_X, CAM_Y, 0.275, -Math.PI / 3, 2 * Math.PI / 3, 10);
  pts.push([-0.145, CAM_Y + 0.238]);
  arc(-CAM_X, CAM_Y, 0.275, Math.PI / 3, 7 * Math.PI / 6, 10);
  pts.push([-0.415, 3.19]);
  arc(0, 0, 0.6, 5 * Math.PI / 6, -Math.PI / 2, 12);
  const chainCurve = new THREE.CatmullRomCurve3(
    pts.map(([x, y]) => new THREE.Vector3(x, y, HALFLEN + 0.62)), true, 'catmullrom', 0.1);
  const chain = new THREE.Mesh(new THREE.TubeGeometry(chainCurve, 220, 0.032, 8, true), M(0x1c1e22, 0.5, 0.7));
  belt.add(chain);
  ...
  addAnchor('正时链', belt, 0.5, CAM_Y - 0.6, HALFLEN + 0.62);
  ...
  function update(theta, explodeT) {
    ...
    const [bx, by, bz] = off(parts.crank);
    ...
    for (let j = 0; j < 4; j++) {
      ...
      pistons[j].position.set(0, wristY + 1.35 * t, ZPOS[j]);
      rods[j].position.set(pinX, pinY + by + 0.75 * t, ZPOS[j]);
    ...
    for (const name of ['block', 'head', 'gasket', 'intake', 'exhaust', 'pan', 'belt', 'plug']) {
